...
Define the component interfaces, layout requirements, and rendering behavior for displaying media review cards in a structured list format. Media reviews are presented as cards with a cover image and metadata (title, excerpt, author, publish date) in a responsive layout suitable for desktop and mobile devices.
