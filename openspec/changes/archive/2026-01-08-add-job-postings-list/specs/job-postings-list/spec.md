...
Defines the job postings list feature for the Ocean Food Group website. Provides components and server actions to display a list of published job postings fetched from Supabase. Includes the JobPostingCard component for individual job display and server-side data fetching. Implements responsive layout, error handling, loading states, and empty state messaging.
...
- JobPostingCard component for displaying individual job postings with support for external application URLs
- JobPostingsList component with infinite scroll pagination (10 items per load)
- Employment type formatting utility for consistent label display
- Server action for fetching only published job postings from Supabase with pagination support (excludes draft and closed)
- Updated careers page to fetch and display job postings list with initial 10 items
- TypeScript type definitions for job posting data
